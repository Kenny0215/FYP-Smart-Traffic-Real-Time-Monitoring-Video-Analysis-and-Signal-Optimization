...
export const DashboardLayout = ({ children, activePage, setPage, user }: { children: React.ReactNode, activePage: Page, setPage: (p: Page) => void,user?: { email?: string, user_metadata?: { full_name?: string } } | null}) => {  const [sidebarOpen, setSidebarOpen] = useState(true);

  const navItems = [
    { id: 'dashboard', label: 'Dashboard', icon: <LayoutDashboard size={20} /> },
    { id: 'upload', label: 'Video Upload', icon: <Upload size={20} /> },
    { id: 'analytics', label: 'Analytics', icon: <BarChart3 size={20} /> },
    { id: 'simulation', label: 'AI Simulation', icon: <TrafficLightIcon size={20} /> },
      ...
      <aside className={cn(
        ...
        <div className="h-full flex flex-col">
          <div className="p-6 flex items-center gap-3">
            ...
            {sidebarOpen && <span className="text-lg font-bold text-white tracking-tight">SmartTraffic</span>}
          </div>
          ...
          <nav className="flex-1 px-4 py-4 space-y-2">
            {navItems.map((item) => (
              ...
              >
                <span className={cn(activePage === item.id ? "text-white" : "text-slate-500 group-hover:text-emerald-500")}>
                  {item.icon}
                </span>
          ...
          <div className="p-4 border-t border-brand-border">
            <button 
              ...
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 hover:bg-rose-500/10 hover:text-rose-500 transition-all"
            >
              <LogOut size={20} />
